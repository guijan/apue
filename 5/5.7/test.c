...
malloc_memfile(void *buffer, size_t size, struct flags flags)
{
	...
	if (size == 0 || flags.err)
		goto errerrno;
	if ((cookie = malloc(sizeof(struct memfile))) == NULL)
		goto err;
	if (buffer == NULL) {
		if (flags.p == 0)
			goto errerrno;
...
writefn(void *cookie, const char *buf, int nbytes)
{
	...
	if (nbytes < 0 ||
	    (
	     memfile->flags.r == 0 && memfile->flags.p == 0 &&
	     memfile->flags.a == 0
	    )
	   ) {
		errno = EINVAL;
...
seekfn(void *cookie, fpos_t offset, int whence)
{
	...
	case SEEK_CUR:
		...
		break;
	case SEEK_END:
		rval = memfile->pos = memfile->size - offset;
		break;
	default:
		errno = EINVAL;
		break;
